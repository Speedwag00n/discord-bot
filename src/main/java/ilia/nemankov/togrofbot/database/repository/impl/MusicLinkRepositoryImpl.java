...
public class MusicLinkRepositoryImpl implements MusicLinkRepository {
    ...
    @Override
    public List<MusicLinkEntity> query(Specification specification, String graphName) {
        return this.query(specification, graphName, null);
    }

    @Override
    public List<MusicLinkEntity> query(Specification specification, String graphName, QuerySettings settings) {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction transaction = session.beginTransaction();
